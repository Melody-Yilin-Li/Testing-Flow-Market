...
for g in range(1, num_groups + 1):
    ...
    for r in range(1, num_rounds - prac_rounds + 1): 
        path = '/Users/YilinLi/Documents/UCSC/Flow Data/cda' + str(g) + '/' + str(r + prac_rounds) + '/1_market.json'
        rnd = pd.read_json(
            path,
        )
    ...
    group_par = []
    for r in range(1, num_rounds - prac_rounds + 1):
        path = '/Users/YilinLi/Documents/UCSC/Flow Data/cda' + str(g) + '/' + str(r + prac_rounds) + '/1_participant.json'
        rnd = pd.read_json(
            path,
        )
        rnd.fillna(0, inplace=True)
        ...
        rnd = rnd[['timestamp', 'id_in_subsession', 'id_in_group', 'inventory', 'rate', 'direction', 'clearing_price', 'clearing_rate', 'cumulative_quantity', 'ce_quantity', 'ce_price']]
    ...
    df.columns = [id_in_subsession, id_in_group, inventory, rate, direction, clearing_price, clearing_rate, cumsum, 'ce_quantity', 'ce_price', 'timestamp']
    df['timestamp'] = df.groupby([id_in_subsession, id_in_group])[id_in_group].cumcount() + 1
    groups_par_cda.append(df)
    

    # plot indivdual transaction quantity
    plt.figure(figsize=(20,5))
    sns.lineplot(data=df[(df[rate] < 20)], x='timestamp', y=inventory, hue=id_in_group, style=direction, drawstyle='steps-pre', legend='full')
    ...
    plt.title('CDA Individual Transacted Quantity vs Time')
    plt.savefig('group_cda_transaction_{}.png'.format(str(g)))
    plt.show()
